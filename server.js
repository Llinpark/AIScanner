...
function resolveDiagnosticReportDirectory() {
  if (process.env.NODE_REPORT_DIRECTORY) return process.env.NODE_REPORT_DIRECTORY;
  try {
...
// Forensics-only: enable Node diagnostic reports for fatal/uncaught failures.
// Does not change HTTP/webhook semantics. Fly has no volume → /tmp is ephemeral.
if (process.report) {
  try {
    const reportDir = resolveDiagnosticReportDirectory();
    ...
    const persist =
      reportDir === '/data' ? 'volume_if_mounted' : 'ephemeral_tmp_UNKNOWN_survive_restart';
    console.log(
      `[PROCESS_FATAL_REPORT] path=${reportDir} persist=${persist} ` +
        `excludeEnv=${Boolean(process.report.excludeEnv)} ` +
        `reportOnFatal=${Boolean(process.report.reportOnFatalError)}`
    );
  } catch (err) {
    console.warn('[PROCESS] diagnostic report config failed:', err && err.message);
  }
}
...
const {
  ...
  safeHeaders,
  logTvStage,
  logWebhookStageTiming,
  ensureRequestId,
  diagnoseTradingViewWebhookBody
} = require('./utils/webhookPipelineDiag');
const { attachTvAckProbe, resolveMachineId } = require('./utils/tvAckLog');
...
async function assertTradingViewWebhook(req, res) {
  ...
  if (!auth.ok) {
    ...
    logTvStage('TV WEBHOOK AUTH FAIL', {
      ...
      signalUuid: bodyMeta.signalUuid,
      ...(authFailReason === 'stored_tv_username_mismatch'
        ? {
            scriptGenerationId: auth.scriptGenerationId || body.scriptGenerationId || null,
            storedTvMasked: auth.storedTvMasked || null,
            claimsTvuMasked: auth.claimsTvuMasked || null
          }
        : {})
    });
    const licenseDiag =
    ...
    if (
      ...
      logTvStage('TV WEBHOOK LICENSE DIAG', {
        ...
        scriptGenerationId: licenseDiag.scriptGenerationId || auth.scriptGenerationId,
        ...
        reason: licenseDiag.reason || authFailReason
      });
    }
...
app.use((req, res, next) => {
  if (isTradingViewWebhookPath(req)) return attachTvAckProbe(req, res, next);
  next();
});
...
app.get('/api/health', (req, res) => {
  ...
  let redis = null;
  try {
    redis = require('./utils/redisClient').getRedisDiagnostics();
  } catch {
    redis = { redisConfigured: false, redisConnected: false };
  }
  // Fly liveness stays 200 when Redis is down so machines are not restarted
  // into the same closed-client hole. Webhook ingest still 503s fail-closed.
  res.status(healthy ? 200 : 503).json({
    ...
    redis: {
      redisConfigured: Boolean(redis?.redisConfigured),
      redisConnected: Boolean(redis?.redisConnected),
      lastSuccessfulRedisOperation: redis?.lastSuccessfulRedisOperation || null,
      lastSuccessfulRedisAt: redis?.lastSuccessfulRedisAt || null,
      lastRedisErrorCode: redis?.lastRedisErrorCode || null,
      lastRedisErrorAt: redis?.lastRedisErrorAt || null,
      redisLatencyMs: redis?.redisLatencyMs ?? null
    }
  });
});
...
// Owner of the process-local recent-signal cache. Writes go through
// TradingViewAlertService.saveSignal which caps at MAX_IN_MEMORY_SIGNALS (400).
...
app.post('/api/webhook/tradingview', attachTvAckProbe, webhookLimiter, async (req, res) => {
  ...
  if (req.tvAckProbe) {
    req.tvAckProbe.symbol = earlyMeta.symbol || '-';
    req.tvAckProbe.tf = earlyMeta.timeframe || '-';
  }
  ...
  try {
    const authStartedNs = process.hrtime.bigint();
    let auth;
    try {
      auth = await assertTradingViewWebhook(req, res);
    } finally {
      if (req.tvAckProbe) {
        req.tvAckProbe.authMs = Number(process.hrtime.bigint() - authStartedNs) / 1e6;
      }
    }
    if (!auth) {
      if (req.tvAckProbe && req.tvAckProbe.outcome == null) {
        req.tvAckProbe.outcome = Number(res.statusCode) >= 500 ? 'error' : 'rejected';
      }
      return;
    }
    ...
    if (parsed?.__parseError) {
      ...
      if (req.tvAckProbe) req.tvAckProbe.outcome = 'rejected';
    ...
    // Correlation id — persisted on Signal.correlation / pipelineRequestId (schema fields).
    if (parsed && typeof parsed === 'object') {
      parsed.pipelineRequestId = requestId;
    }
    const meta = extractPipelineMeta(parsed);
    if (req.tvAckProbe) {
      req.tvAckProbe.symbol = meta.symbol || req.tvAckProbe.symbol || '-';
      req.tvAckProbe.tf = meta.timeframe || req.tvAckProbe.tf || '-';
    }
    ...
    if ((isCandlePayload || isCandleFeed) && parsed.symbol && !isStructuredEntry) {
      ...
      if (req.tvAckProbe) {
        req.tvAckProbe.outcome = 'candle_ack';
        req.tvAckProbe.accepted = false;
        req.tvAckProbe.persisted = false;
        req.tvAckProbe.deferredFanout = false;
        req.tvAckProbe.skippedFanout = true;
        req.tvAckProbe.reason = 'candle_ack';
      }
      return res.status(201).json({
        success: true,
        mode: 'candle_ack',
    ...
    const acceptStartedNs = process.hrtime.bigint();
    ...
    if (req.tvAckProbe) {
      req.tvAckProbe.acceptMs = Number(process.hrtime.bigint() - acceptStartedNs) / 1e6;
    }
    logWebhookStageTiming({
      requestId,
      stage: 'accept',
      durationMs: Date.now() - acceptStartedAt,
      success: !accept?.rejected,
      errorCode: accept?.redisErrorCode || accept?.reason || '-'
    });
    ...
    if (accept?.rejected) {
      logPipeline('Publish', 'FAIL', {
        ...
        reason: `${accept.reason || 'rejected'}; redisErrorCode=${accept.redisErrorCode || '-'}; requestId=${requestId}`
      });
      ...
      return res.status(status).json({
        ...
        redisErrorCode: accept.redisErrorCode || undefined,
    ...
    logPipeline('Accepted', 'PASS', {
      ...
      reason: accept.duplicate
        ...
        : accept.pendingOutcome
          ? `pending_outcome; not_persisted_trade; latencyMs=${latencyMs}; requestId=${requestId}`
          : accept.persisted
            ? `durable_ack; persisted=true; latencyMs=${latencyMs}; requestId=${requestId}`
            : `accepted_no_persist; reason=${accept.reason || 'n/a'}; latencyMs=${latencyMs}; requestId=${requestId}`
    });
    ...
    if (req.tvAckProbe) {
      const skipped = Boolean(accept.duplicate || accept.skippedFanout);
      req.tvAckProbe.outcome = skipped ? String(accept.reason || 'skipped_fanout') : 'accepted';
      req.tvAckProbe.accepted = Boolean(accept.accepted) && !skipped;
      req.tvAckProbe.persisted = Boolean(accept.persisted);
      req.tvAckProbe.deferredFanout = !skipped;
      req.tvAckProbe.skippedFanout = skipped;
      req.tvAckProbe.reason = accept.reason || (skipped ? 'skipped_fanout' : 'accepted');
      req.tvAckProbe.signalUuid = accept.signalUuid || meta.signalUuid;
      req.tvAckProbe.eventId = parsed.eventId || accept.signalData?.eventId;
      req.tvAckProbe.canonicalTradeId = parsed.canonicalTradeId || accept.signalUuid;
    }
    return res.status(202).json({
      ok: true,
      accepted: true,
      persisted: Boolean(accept.persisted),
      pendingOutcome: Boolean(accept.pendingOutcome),
      success: true,
      publishOnly: true,
      requestId,
      signalUuid: accept.signalUuid || meta.signalUuid,
      latencyMs,
      duplicate: Boolean(accept.duplicate),
      skippedFanout: Boolean(accept.duplicate || accept.skippedFanout),
    ...
    if (req.tvAckProbe) req.tvAckProbe.outcome = isPersistFail ? 'error' : 'rejected';
    if (isPersistFail) {
...
app.get('/api/performance/summary', requireAuth, requireSubscription, requireTierFeature('performanceDashboard'), async (req, res) => {
  try {
    const analytics = await createAnalyticsRouter.loadCanonicalAnalytics({
      user: req.user,
      inMemorySignals,
      isDbReady
    });
...
server.on('listening', () => {
  ...
  console.log(
    `[PROCESS] pid=${process.pid} node=${process.version} ` +
      `machine=${resolveMachineId()} platform=${process.platform} ` +
      `reportDir=${(process.report && process.report.directory) || '/tmp'} ` +
      `reportOnFatal=${Boolean(process.report && process.report.reportOnFatalError)} ` +
      `reportOnUncaught=${Boolean(process.report && process.report.reportOnUncaughtException)}`
  );

  // Production: Redis is preferred for MT5 PairCode storage. Temporary Redis
  // unavailability must not kill the already-listening HTTP server (degraded mode).
  try {
    ...
    Mt5PairingService.assertProductionRedisReady().catch(err => {
      ...
      const isRedisUnavailable =
        code === 'PAIRING_UNAVAILABLE' ||
        /PAIRING_UNAVAILABLE/i.test(msg) ||
        /Redis unavailable/i.test(msg) ||
        /Redis unavailable/i.test(detail);
      if (isRedisUnavailable) {
        console.error('[Mt5Pairing] Production Redis check failed:', err && err.message);
        console.error('[Mt5Pairing] API continuing in DEGRADED mode; MT5 pairing temporarily unavailable.');
        return;
      }
      console.error('[Mt5Pairing] Production Redis check failed:', err && err.message);
  ...
  // Background crash recovery only — live webhook fan-out does not wait on this worker.
  // Does NOT start djobidx cleanup; that is an explicit admin script (scripts/cleanup-djobidx.js).
  try {
...
function handleProcessShutdown(signal) {
  try {
    ...
    console.warn(`[Shutdown] redis ${signal}:`, err?.message || err);
  }
}
