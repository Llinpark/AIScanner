...
export const subscriptionApi = {
  ...
  getPerformanceSummary: () => api.get('/api/analytics/summary')
};
...
export const adminApi = {
  ...
  getPipelineRequestOutcome: (params = {}) =>
    api.get('/api/admin/pipeline-request-outcome', { params }),
  getSignalCorrelation: (params = {}) => {
    if (params.requestId && Object.keys(params).length === 1) {
      return api.get(`/api/admin/signal-correlation/${encodeURIComponent(params.requestId)}`);
    }
    return api.get('/api/admin/signal-correlation', { params });
  }
};
