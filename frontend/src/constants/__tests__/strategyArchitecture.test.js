...
describe('frontend Strategy Architecture', () => {
  ...
  it('exposes chart hints and future slots', () => {
    ...
    assert.match(getStrategyArchitecture('scalping').chartHint, /Pine 1\.3\.0/);
    assert.match(getStrategyArchitecture('scalping').chartHint, /Prefer ONE TradingView alert on canonical 3m/i);
    assert.doesNotMatch(getStrategyArchitecture('scalping').chartHint, /visualization-only/i);
    assert.doesNotMatch(getStrategyArchitecture('scalping').chartHint, /1\.6\.0/);
    ...
    assert.match(getStrategyArchitecture('daytrading').chartHint, /Pine 1\.3\.0/);
    assert.match(getStrategyArchitecture('daytrading').chartHint, /Prefer ONE TradingView alert on canonical 5m/i);
    assert.doesNotMatch(getStrategyArchitecture('daytrading').chartHint, /visualization-only/i);
    assert.doesNotMatch(getStrategyArchitecture('daytrading').chartHint, /1\.6\.0/);
