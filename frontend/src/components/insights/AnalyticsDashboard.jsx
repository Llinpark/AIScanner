  ...
  useEffect(() => {
    ...
    analyticsApi
      ...
      .then(summaryRes => {
        ...
        setTimeseries({
          timeseries: summaryRes.data?.timeseries,
          equityCurve: summaryRes.data?.equityCurve,
          patternStats: summaryRes.data?.patternStats
        });
      })
      .catch(err => {
      ...
      <div className="analytics-panels">
        ...
        <div className="analytics-panel analytics-panel--table">
          ...
          {strategyRows.length === 0 ? (
            ...
            <div className="strategy-perf-grid" role="table" aria-label="Strategy performance">
              <div className="strategy-perf-row strategy-perf-row--head" role="row">
                <span role="columnheader">Strategy</span>
                <span role="columnheader">Trades</span>
                <span role="columnheader">Win rate</span>
                <span role="columnheader">Avg R</span>
              </div>
              {strategyRows.map(row => (
                <div className="strategy-perf-row" role="row" key={row.key || row.pattern || row.label}>
                  <span role="cell">{row.label || row.pattern}</span>
                  <span role="cell">{row.total}</span>
                  <span role="cell">{row.winRate}%</span>
                  <span role="cell">{row.avgR}R</span>
                </div>
              ))}
            </div>
          )}
