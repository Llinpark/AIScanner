...
describe('RC-G independent ENTRY channel delivery', () => {
  const originalFetch = global.fetch;
  ...
  function premiumAutoSub(overrides = {}) {
  ...
  beforeEach(() => {
    ...
    originalMailer = mailer.sendTradeAlertEmail;
    originalQueue = Mt5TradeCopierService.queueExecutionForUser;
  });

  afterEach(() => {
    global.fetch = originalFetch;
    mailer.sendTradeAlertEmail = originalMailer;
  ...
  it('TEST C: slow email does not delay Telegram or MT5 start', async () => {
    const timings = {};
    ...
    await TradeDeliveryService.deliverToSubscriber(
      io,
      entrySignal({ _id: 'mem_rcg_c', signalUuid: 'uuid-rcg-c', entryAcceptedAt: new Date() }),
      premiumAutoSub()
    );

    assert.ok(timings.t1 >= timings.t0, 'email attempt must start at or after T0');
    assert.ok(timings.t2, 'Telegram attempt must start');
    assert.ok(timings.t3, 'MT5 attempt must start');
    assert.ok(timings.t4, 'email must complete');
    assert.ok(
      timings.t2 < timings.t4,
      `T2 Telegram start ${timings.t2} must occur before T4 email complete ${timings.t4}`
    );
    assert.ok(
      timings.t3 < timings.t4,
      `T3 MT5 start ${timings.t3} must occur before T4 email complete ${timings.t4}`
    );
    assert.ok(timings.t2 - timings.t0 < 1500, 'Telegram must not wait for the 3s email delay');
    assert.ok(timings.t3 - timings.t0 < 1500, 'MT5 must not wait for the 3s email delay');
  });

  it('TEST D: email failure does not prevent Telegram or MT5; email failure is recorded', async () => {
    mailer.sendTradeAlertEmail = async () => ({ ok: false, reason: 'smtp_down' });
    let tgAttempts = 0;
    let mt5Attempts = 0;
    global.fetch = async () => {
      tgAttempts += 1;
    ...
    const io = { to: () => ({ emit() {} }), emit() {} };
    const signal = entrySignal({ _id: 'mem_rcg_d', signalUuid: 'uuid-rcg-d', entryAcceptedAt: new Date() });
    await TradeDeliveryService.deliverToSubscriber(io, signal, premiumAutoSub());

    assert.equal(tgAttempts, 1);
    assert.equal(mt5Attempts, 1);
    const emailJob = await DurableDelivery.getJobBySpec(
      deliveryIdempotency.specFrom(signal, 'entry', 'email', 'prem_1')
    );
    assert.ok(emailJob);
    assert.notEqual(emailJob.state, 'delivered');
    assert.match(String(emailJob.lastError || emailJob.state || ''), /smtp_down|retry|failed/i);
    const tgJob = await DurableDelivery.getJobBySpec(
      deliveryIdempotency.specFrom(signal, 'entry', 'telegram', 'prem_1')
    );
    assert.equal(tgJob.state, 'delivered');
  });

  it('TEST E: Telegram failure does not unnecessarily block MT5; failure semantics stay truthful', async () => {
    mailer.sendTradeAlertEmail = async () => ({ ok: true });
    let mt5Attempts = 0;
    global.fetch = async () => {
      throw new Error('telegram_boom');
    };
    Mt5TradeCopierService.queueExecutionForUser = async () => {
      mt5Attempts += 1;
      return { ok: true };
    ...
    const io = { to: () => ({ emit() {} }), emit() {} };
    const signal = entrySignal({ _id: 'mem_rcg_e', signalUuid: 'uuid-rcg-e', entryAcceptedAt: new Date() });
    await TradeDeliveryService.deliverToSubscriber(io, signal, premiumAutoSub());

    assert.equal(mt5Attempts, 1, 'MT5 must proceed despite Telegram failure');
    const tgJob = await DurableDelivery.getJobBySpec(
      deliveryIdempotency.specFrom(signal, 'entry', 'telegram', 'prem_1')
    );
    assert.ok(tgJob);
    assert.notEqual(tgJob.state, 'delivered');
    const mt5Job = await DurableDelivery.getJobBySpec(
      deliveryIdempotency.specFrom(signal, 'entry', 'mt5', 'prem_1')
    );
    assert.equal(mt5Job.state, 'delivered');
    const status = TradeDeliveryService.resolveDeliveryStatus({
      telegramSent: false,
      mt5Sent: true,
      tgPipelineStatus: 'FAIL',
  ...
  it('TEST H: MT5 remains ENTRY-only', async () => {
    mailer.sendTradeAlertEmail = async () => ({ ok: true });
    let mt5Attempts = 0;
    global.fetch = async () => okSend();
    Mt5TradeCopierService.queueExecutionForUser = async () => {
      mt5Attempts += 1;
      return { ok: true };
    };
    ...
    const io = { to: () => ({ emit() {} }), emit() {} };
    const tp = entrySignal({
      _id: 'mem_rcg_h',
      signalUuid: 'uuid-rcg-h',
      alertType: 'take_profit_1'
    });
    const mt5 = await TradeDeliveryService.deliverMt5Auto(premiumAutoSub(), tp);
    assert.equal(mt5.ok, false);
    assert.equal(mt5.reason, 'not_entry_signal');
    assert.equal(mt5Attempts, 0);

    await TradeDeliveryService.deliverToSubscriber(io, tp, premiumAutoSub());
    assert.equal(mt5Attempts, 0);
  });
});
