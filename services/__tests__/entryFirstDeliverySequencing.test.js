...
describe('entry-first delivery sequencing 1â€“20', () => {
  ...
  it('TEST G: independent ENTRY channels must not let TP/SL overtake ENTRY (sequencer unchanged)', async () => {
    assert.deepEqual(DeliverySequencer.requiredPredecessors('take_profit_3', ['entry']), ['entry']);
    assert.deepEqual(DeliverySequencer.requiredPredecessors('stop_loss', ['entry']), ['entry']);

    const uuid = 'seq-rcg-g';
    const entryAccept = await acceptOnly(io, payload('entry', uuid, { symbol: 'SEQG' }), mem);
    const tp3Accept = await acceptOnly(io, payload('take_profit_3', uuid, { symbol: 'SEQG' }), mem);
    const slAccept = await acceptOnly(io, payload('stop_loss', uuid, { symbol: 'SEQG' }), mem);
    assert.equal(entryAccept.accepted, true);
    ...
    await waitUntil(() => tg.started.some(isEntryText));
    await new Promise(r => setTimeout(r, 40));
    assert.equal(tg.started.some(isTp3Text), false, 'TP3 must not start while ENTRY is in-flight');
    assert.equal(
      tg.started.some(t => /SL HIT|TRADE CLOSED/i.test(t)),
      false,
      'SL must not start while ENTRY is in-flight'
    );
    ...
    assert.equal(isEntryText(tg.texts[0]), true);
    const entryIdx = tg.texts.findIndex(isEntryText);
    const tp3Idx = tg.texts.findIndex(isTp3Text);
    if (tp3Idx >= 0) assert.ok(entryIdx < tp3Idx, 'ENTRY Telegram must precede TP3');
  });
});
