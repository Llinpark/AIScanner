...
async function processIncomingTradeAlert(baseData, inMemorySignals = [], options = {}) {
  ...
  if (isOutcomeAlert(alertType) && updatedEntry?._outcomeIgnored && !updatedEntry?._suppressDelivery) {
    return {
      rejected: false,
      skipped: true,
  ...
  // SL after TP: registry is cleared, but never fan out SL HIT.
  if (updatedEntry?._suppressDelivery) {
    return {
      rejected: false,
      skipped: true,
      reason: updatedEntry._outcomeIgnoreReason || SL_AFTER_HIGHEST_TP,
