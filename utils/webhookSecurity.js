...
/** Safe log fragment only — never a full username or token. */
function maskTvUsername(value) {
    ...
    if (claims) {
      ...
      if (resolveUserById) {
        ...
        if (!storedTv || storedTv !== claims.tvu) {
          return {
            ok: false,
            reason: 'stored_tv_username_mismatch',
            licenseDiagnostics,
            body,
            storedTvMasked: maskTvUsername(storedTv),
            claimsTvuMasked: maskTvUsername(claims.tvu),
            scriptGenerationId:
              licenseDiagnostics?.scriptGenerationId ||
