...
describe('verifyTradingViewWebhook', () => {
  ...
  it('rejects stored_tv_username_mismatch with masked tvu only (still unauthorized, no auth bypass)', async () => {
    const userId = '64b0f0f0f0f0f0f0f0f0f0e5';
    const claimsTvu = 'chartuseralpha';
    const storedTvu = 'accountuserbeta';
    const licenseToken = generateLicenseToken(userId, claimsTvu);
    ...
    const auth = await verifyTradingViewWebhook(
      reqWithBody({
        ...
        userId,
        tradingviewUsername: claimsTvu,
        licenseToken,
        scriptGenerationId: 'a51cbe10bbff4cdf'
      }),
      async () => ({
        _id: userId,
        tradingviewUsername: storedTvu,
        subscription: { status: 'active', tier: 'professional' }
      })
    );

    assert.equal(auth.ok, false);
    assert.equal(auth.reason, 'stored_tv_username_mismatch');
    assert.match(String(auth.storedTvMasked), /\*\*\*/);
    assert.match(String(auth.claimsTvuMasked), /\*\*\*/);
    assert.ok(!String(auth.storedTvMasked).includes(storedTvu));
    assert.ok(!String(auth.claimsTvuMasked).includes(claimsTvu));
    assert.equal(auth.scriptGenerationId, 'a51cbe10bbff4cdf');
    const { maskTvUsername } = require('../webhookSecurity');
    assert.equal(maskTvUsername(storedTvu), auth.storedTvMasked);
    assert.equal(maskTvUsername(claimsTvu), auth.claimsTvuMasked);
    assert.equal(maskTvUsername(''), '(empty)');
  });
