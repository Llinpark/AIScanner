...
function logWebhookStageTiming({
  requestId,
  stage,
  durationMs,
  success,
  ...
  console.log(
    `[WEBHOOK_STAGE_TIMING] requestId=${requestId || 'n/a'} ` +
      `machineId=${process.env.FLY_MACHINE_ID || 'n/a'} stage=${stage || 'n/a'} ` +
      `durationMs=${Number.isFinite(durationMs) ? durationMs : 'n/a'} ` +
      `success=${success === false ? 'false' : 'true'} errorCode=${errorCode || '-'}`
  );
}
