...
/** Mongo match for open ENTRY trades — TP1/TP2 partials stay open, not closed. */
function openEntryMongoFilter() {
  return {
    ...
    $and: [
      {
        $or: [
          ...
          { tradeStatus: { $exists: false } },
          { tradeStatus: null }
        ]
      },
      {
        $or: [
          { outcome: { $in: ['pending', 'tp1', 'tp2'] } },
...
/**
 * Authoritative highest take-profit reached. SL is not a target.
 * NONE < TP1 < TP2 < TP3
 */
function highestTargetReached(entry) {
  if (!entry || typeof entry !== 'object') return 'none';
  const a = milestoneRank(entry.highestMilestone);
  const b = milestoneRank(entry.outcome);
...
const SL_AFTER_HIGHEST_TP = 'sl_after_highest_tp';
...
function isNonUpgradableTerminal(entrySignal) {
  const outcome = String(entrySignal?.outcome || '').toLowerCase();
  const ts = String(entrySignal?.tradeStatus || '').toLowerCase();
  return (
    outcome === 'expired' ||
    outcome === 'cancelled' ||
    ts === 'expired' ||
...
function ignoreOutcome(entrySignal, reason, incomingOutcome, alertType) {
  logOutcomeIgnored(entrySignal, reason, incomingOutcome, alertType);
  entrySignal._outcomeIgnored = true;
  entrySignal._suppressDelivery = false;
  entrySignal._outcomeIgnoreReason = reason;
  return entrySignal;
}

function closePreservingHighestTp(entrySignal, highestTp, alertType) {
  entrySignal.outcome = highestTp;
  entrySignal.outcomeR = OUTCOME_R[highestTp] ?? 0;
  entrySignal.lifecycleStage = lifecycleStageFromOutcome(highestTp);
  entrySignal.highestMilestone = maxMilestone(entrySignal.highestMilestone, highestTp);
  entrySignal.closedReason = `sl_after_${highestTp}`;
  entrySignal.tradeStatus = 'won';
  entrySignal.closedAt = entrySignal.closedAt || new Date();
  entrySignal._suppressDelivery = true;
  entrySignal._outcomeIgnored = false;
  entrySignal._outcomeIgnoreReason = SL_AFTER_HIGHEST_TP;
  logOutcomeIgnored(entrySignal, SL_AFTER_HIGHEST_TP, 'sl', alertType);
  return entrySignal;
}

function upgradeSlToHighestTp(entrySignal, tpOutcome) {
  entrySignal.outcome = tpOutcome;
  entrySignal.outcomeR = OUTCOME_R[tpOutcome] ?? 0;
  entrySignal.lifecycleStage = lifecycleStageFromOutcome(tpOutcome);
  entrySignal.highestMilestone = maxMilestone(entrySignal.highestMilestone, tpOutcome);
  entrySignal.closedReason = `sl_after_${tpOutcome}`;
  entrySignal.tradeStatus = 'won';
  entrySignal.closedAt = entrySignal.closedAt || new Date();
  entrySignal._suppressDelivery = false;
  entrySignal._outcomeIgnored = false;
 ...
 * Milestone progression is monotonic: NONE < TP1 < TP2 < TP3.
 * SL_HIT is the final outcome only when no take-profit was ever reached.
 * A later SL never downgrades TP1/TP2/TP3. Concurrent TP after SL upgrades to that TP.
 *
 * Sets ephemeral flags (not persisted):
 *   _outcomeIgnored {boolean} — no state change
 *   _suppressDelivery {boolean} — state changed (close) but do not notify SL HIT
...
function applyOutcomeUpdate(entrySignal, alertType, closedReason) {
  ...
  const existingStatus = String(entrySignal.tradeStatus || '').toLowerCase();

  entrySignal._outcomeIgnored = false;
  entrySignal._suppressDelivery = false;
  entrySignal._outcomeIgnoreReason = null;

  if (isNonUpgradableTerminal(entrySignal)) {
    const reason =
      existingOutcome === outcome || String(entrySignal.lifecycleStage || '') === lifecycleStageFromOutcome(outcome)
        ? 'already_terminal_same'
        : 'already_terminal';
    return ignoreOutcome(entrySignal, reason, outcome, alertType);
  }

  const currentHighest = highestTargetReached(entrySignal);
  const incomingTpRank = WIN_OUTCOMES.has(outcome) ? milestoneRank(outcome) : 0;
  const currentTpRank = milestoneRank(currentHighest);

  // SL: loss only if no TP; otherwise close while preserving highest TP.
  if (outcome === 'sl') {
    if (WIN_OUTCOMES.has(currentHighest)) {
      if (isTerminalEntry(entrySignal) && existingStatus === 'won') {
        return ignoreOutcome(entrySignal, 'already_terminal', outcome, alertType);
      }
      return closePreservingHighestTp(entrySignal, currentHighest, alertType);
    }
    if (isTerminalEntry(entrySignal)) {
      const reason = existingOutcome === 'sl' ? 'already_terminal_same' : 'already_terminal';
      return ignoreOutcome(entrySignal, reason, outcome, alertType);
    }
  }

  // Incoming TP: never move backward; may upgrade a persisted SL (concurrent / out of order).
  if (WIN_OUTCOMES.has(outcome)) {
    if (incomingTpRank < currentTpRank) {
      return ignoreOutcome(entrySignal, 'backward_transition', outcome, alertType);
    }
    if (incomingTpRank === currentTpRank && currentTpRank > 0) {
      const reason = PARTIAL_OUTCOMES.has(outcome) ? 'same_stage_replay' : 'already_terminal_same';
      return ignoreOutcome(entrySignal, reason, outcome, alertType);
    }
    if (existingOutcome === 'sl' || existingStatus === 'lost') {
      return upgradeSlToHighestTp(entrySignal, outcome);
    }
  }

  // Immutable once terminal (TP3 / remaining SL / won|lost) when the incoming event cannot upgrade.
  if (isTerminalEntry(entrySignal)) {
    const reason =
      existingOutcome === outcome || String(entrySignal.lifecycleStage || '') === lifecycleStageFromOutcome(outcome)
        ? 'already_terminal_same'
  ...
  if (PARTIAL_OUTCOMES.has(outcome) && existingOutcome === outcome) {
    return ignoreOutcome(entrySignal, 'same_stage_replay', outcome, alertType);
  }
