...
const { openEntryMongoFilter } = require('../utils/signalOutcome');
...
function createAdminRouter({ io } = {}) {
  ...
  router.get('/pipeline-status', requireSuperAdmin, async (req, res) => {
    ...
    const degraded = (err, extra = {}) => {
      ...
      return {
        ok: false,
        status: 'degraded',
        message: 'Pipeline diagnostics degraded',
        error: err?.message || 'timeout_or_unavailable',
        redis: {
          status: redis.redisConnected ? 'ok' : redis.redisConfigured ? 'unavailable' : 'not_configured',
          error: redis.redisLastError || redis.lastRedisError || err?.message || null,
          errorCode: redis.lastRedisErrorCode || null,
          redisConfigured: Boolean(redis.redisConfigured),
          redisClientOpen: Boolean(redis.redisClientOpen),
          redisClientReady: Boolean(redis.redisClientReady),
          redisConnectionState: redis.redisConnectionState || null,
          redisLastError: redis.redisLastError || redis.lastRedisError || err?.message || null,
          redisReconnectInProgress: Boolean(redis.redisReconnectInProgress)
        },
        mongo: {
    ...
    try {
      ...
      const status = await withTimeout(
        PipelineStatusService.getStatus({
      ...
      res.status(200).json(degraded(error));
    }
  });
  ...
  router.get('/live-pipeline', requireSuperAdmin, async (req, res) => {
    ...
    try {
      ...
      const live = await withTimeout(
        PipelineStatusService.getLivePipeline(limit),
        4000,
        'admin_live_pipeline'
      );
      res.json(live);
      ...
      res.status(200).json({
        ...
        events: [],
        count: 0,
        message: 'Live pipeline diagnostics degraded',
        error: error.message
      });
    }
  });
  ...
  router.get('/pipeline-subscribers', requireSuperAdmin, async (req, res) => {
    ...
    try {
      ...
      const result = await withTimeout(
        PipelineAlertStatusService.listActiveSubscriberAlertStatus(),
        4000,
        'admin_pipeline_subscribers_route'
      );
      ...
      res.status(200).json({
        ...
        activeSubscribers: 0,
        waitingSubscribers: 0,
        message: 'Unable to load pipeline subscribers',
        error: error.message
      });
    }
  });
  ...
  router.get('/delivery-stats', requireSuperAdmin, async (req, res) => {
    ...
    try {
      ...
      res.status(200).json({
        ok: false,
        status: 'degraded',
        message: 'Unable to load delivery stats',
        error: error.message
      });
    }
  });

  router.get('/pipeline-request-outcome', requireSuperAdmin, async (req, res) => {
    const { withTimeout } = require('../utils/boundedWait');
    try {
      const PipelineDeliveryStatsService = require('../services/PipelineDeliveryStatsService');
      const result = await withTimeout(
        PipelineDeliveryStatsService.findRequestOutcome({
          requestId: req.query.requestId,
          signalUuid: req.query.signalUuid,
          eventId: req.query.eventId,
          canonicalTradeId: req.query.canonicalTradeId,
          symbol: req.query.symbol,
          subscriber: req.query.subscriber
        }),
        4000,
        'admin_pipeline_request_outcome'
      );
      res.json(result);
    } catch (error) {
      console.error('Admin pipeline-request-outcome error:', error);
      res.status(200).json({
        ok: false,
        status: 'degraded',
        message: 'Unable to load request outcome',
        error: error.message,
        signals: [],
        jobs: [],
        timeline: [],
  ...
  router.get('/signal-correlation/:requestId', requireSuperAdmin, async (req, res) => {
    req.query.requestId = req.params.requestId || req.query.requestId;
    const { withTimeout } = require('../utils/boundedWait');
    try {
      const SignalCorrelationService = require('../services/SignalCorrelationService');
      const result = await withTimeout(
        SignalCorrelationService.lookupCorrelation({
          requestId: req.params.requestId || req.query.requestId,
          signalUuid: req.query.signalUuid,
          eventId: req.query.eventId,
          canonicalTradeId: req.query.canonicalTradeId,
          symbol: req.query.symbol,
          subscriberId: req.query.subscriber
        }),
        4000,
        'admin_signal_correlation'
      );
      res.json(result);
    } catch (error) {
      console.error('Admin signal-correlation error:', error);
      res.status(200).json({
        ok: false,
        diagnosticOnly: true,
        message: 'Unable to load correlation',
        error: error.message,
        timeline: [],
        firstFailingBoundary: null
      });
  ...
  router.get('/signal-correlation', requireSuperAdmin, async (req, res) => {
    const { withTimeout } = require('../utils/boundedWait');
    try {
      const SignalCorrelationService = require('../services/SignalCorrelationService');
      const result = await withTimeout(
        ...
        4000,
        'admin_signal_correlation_query'
      );
      res.json(result);
    } catch (error) {
      console.error('Admin signal-correlation error:', error);
  ...
  router.get('/stats', async (req, res) => {
    try {
      ...
      if (dbConnected) {
        const todayUtc = PipelineDeliveryStatsService.startOfDay();
        const entryTodayMatch = {
        ...
        const [
          ...
          Signal.countDocuments({ alertType: { $in: ['entry', 'signal'] }, selfTest: { $ne: true } }),
          PipelineDeliveryStatsService.countUniqueCanonicalSignals(entryTodayMatch),
          Signal.countDocuments(openEntryMongoFilter()),
