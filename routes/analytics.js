...
async function loadCanonicalAnalytics({ user, inMemorySignals, isDbReady }) {
  const cutoff = historyCutoffDate(user.subscription);
  const raw = isDbReady()
    ? await Signal.find({
  ...
  const filtered = filterSignalsForTier(raw, user.subscription).filter(isWebhookInsightsSignal);
  const analytics = buildAnalytics(filtered);
  return {
    ...analytics,
    historyDays: getTierFeatures(user.subscription).historyDays
  };
}
...
createAnalyticsRouter.loadCanonicalAnalytics = loadCanonicalAnalytics;
module.exports = createAnalyticsRouter;
module.exports.loadCanonicalAnalytics = loadCanonicalAnalytics;
