...
function devUserToSubscriber(record) {
  if (!record || !canAccessTradingViewAlerts(record.subscription)) return null;
  return {
    username: record.username,
    tradingviewUsername: record.tradingviewUsername,
...
async function findSubscriberByTvUsername(tradingviewUsername) {
  ...
  if (useDevUserStore()) {
    return devUserToSubscriber(devUserStore.findByTradingViewUsername(normalized));
  }
